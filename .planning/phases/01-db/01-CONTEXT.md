...
- **D-07:** 디자인의 대기시간(waitSeconds)은 제품에서 **내부 상수**로 고정(가짜 배달 대기는 Phase 3). 디자인 기본 13초를 출발점으로, 실제 값은 Phase 3 재량.

### 진입 경험 & Phase 1 가시성 (MVP 가시 가치)
- **D-08:** 미니앱 첫 진입은 **1회성 환영 인트로** — "시켜놓고, 참는다" 톤의 한 장 인트로를 **첫 방문에만** 보여주고 이후엔 바로 홈.
- **D-09:** "첫 방문" 판정은 가벼운 클라이언트 플래그(localStorage)로 충분 — 서버 가치 없음. (planner 재량이나 DB 컬럼까지 갈 필요 없음.)
